...
//	}
     ...
     */
    public static int[][] getPDforSnappy(BufferedReader br) throws java.io.IOException {
        // System.out.print("Enter PD: ");
        String str = "";
        String line;
        while (true) {
     ...
     *
     *
     *	考虑 mats 和 colcopy 私有化
     *  添加序列化方法
     *  this 传递给 toSmith 方法, 从中序列化
     *
     */
    private int smithState;
    private         int ncolForSmith;
    ...
    public void SaveIntMatState(int param, String str)
    {
        ...
        if ((curTime - lastTime)/1000<60*autoSave || autoSave == 0) //超过10分钟则存档
        //if ((curTime - lastTime)200 || autoSave == 0) //超过10分钟则存档
            return;
        else
            lastTime = curTime;

        File output = new File("cache/Smith/" + new Integer(param).toString()+str);
        output.getParentFile().mkdirs();
        try {
            System.out.print("\rwriting cache: " + output.getName() + "...                           ");
            ObjectOutputStream serializer = new ObjectOutputStream(
                    new BufferedOutputStream(new FileOutputStream(output), 1024*1024*512));
            //do write
            serializer.writeObject(KhZret);

            //serializer.writeInt(colcopy.size());
            for (int i = 0; i < colcopy.size(); i++) {
                //serializer.writeInt(colcopy.get(i).size());
                for (int j = 0; j < colcopy.get(i).size(); j++) {
                    serializer.writeInt(colcopy.get(i).get(j));
                }
            }

            //serializer.writeInt(mats.length);
            for (int col = 0; col < mats.length; col++) {
                //serializer.writeInt(mats[col].rows);
                //serializer.writeInt(mats[col].columns);
                for (int i = 0; i < mats[col].rows; i++) {
                    System.out.print("\rwriting mat: "+ col +"/"+mats.length+"\t"+i+"/"+mats[col].rows+"...          ");
                    for (int j = 0; j < mats[col].columns; j++)
                        serializer.writeObject(mats[col].matrix[i][j]);
                }
...
//            File output = new File("cache/Smith/" + new Integer(i).toString());
//            if (output.exists()){
//                maxSaved = i;
//                break;
//            }
//        }
        ...
        File path = new File("cache/Smith/");
        ...
        if (path.exists()) {
            ...
            for (File f :
                ...
                try {
                    int pair[] = new int[2];
                    pair[0] = Integer.parseInt(n[0]);
                    pair[1] = Integer.parseInt(n[1]);
                    a.add(pair);
                } catch (Exception e) {
                    continue;
                }
        ...
        for (int i = ncolForSmith - 1; i >= param && maxSaved == -1; i--) {
            for (int j = 0; j < a.size(); j++) {
                if (((int[])a.get(j))[0] == i)
                {
                    maxSaved = ((int[])a.get(j))[0];
                    break;
                }
            }
        }
        ...
        for (int i = 0; i < a.size(); i++) {
            int[] x = ((int[])a.get(i));
            if (x[0] == maxSaved && x[1] > maxSmithParam)
                maxSmithParam = x[1];
        }

        if (maxSmithParam!=-1)
            smithState = maxSmithParam;
        else
            smithState = 0;
        ...
        File input = new File("cache/Smith/" + new Integer(maxSaved).toString()+"_"+ new Integer(smithState).toString());
        try{
            System.out.println("\rreading cache: "+ input.getName() + "...                           ");
            ObjectInputStream serializer = new ObjectInputStream(
                    new BufferedInputStream(new FileInputStream(input), 1024*1024*512));
            //do read
            KhZret = (String)serializer.readObject();

            //assert colcopy.size() == serializer.readInt();

            for (int i = 0; i < colcopy.size(); i++) {
                //assert colcopy.get(i).size() == serializer.readInt();
                for (int j = 0; j < colcopy.get(i).size(); j++) {
                    colcopy.get(i).set(j, serializer.readInt());
                }
            }

            //assert mats.length == serializer.readInt();
            for (int col = 0; col < mats.length; col++) {
                //assert mats[col].rows == serializer.readInt();
                //assert mats[col].columns == serializer.readInt();
                for (int i = 0; i < mats[col].rows; i++) {
                    System.out.print("\rread mat: "+ col +"/"+mats.length+"\t"+i+"/"+mats[col].rows+"                  ");
                    for (int j = 0; j < mats[col].columns; j++)
                        mats[col].matrix[i][j] = (BigInteger) serializer.readObject();
                }
    ...
    public String KhForZ() {
        ...
        if (head.exists() ){
            try {
                ObjectInputStream serializer = new ObjectInputStream(
                        new FileInputStream(head));
                //do read

                colcopy = new ArrayList<List<Integer>>();
                ncolForSmith = serializer.readInt();
                for (int i = 0; i < ncolForSmith; i++) {
                    colcopy.add(new ArrayList<Integer>());
                    int numbers = serializer.readInt();
                    for (int j = 0; j < numbers; j++) {
                        colcopy.get(i).add(0);
                    }
                }

                int matsize = serializer.readInt();
                mats = new IntMatrix[matsize];
                for (int i = 0; i < matsize; i++) {
                    int r = serializer.readInt();
                    int c = serializer.readInt();
                    mats[i] = new IntMatrix(r, c);

                    if (i > 0) {
                        mats[i].prev = mats[i - 1];							//前一个已经算完了, 不需要?
                        //注意如果要smith存档则需要设置 prev
                        mats[i - 1].next = mats[i];
                    }
                }
            ...
            mats = new IntMatrix[matrices.size()];                //mats 和 colcopy 是需要考虑序列化的部分
            colcopy = new ArrayList<List<Integer>>();                //mats 不需要序列化 source, target, prev, next 成员
            for (int i = 0; i < ncolumns; i++) {
                colcopy.add(new ArrayList<Integer>(columns[i].numbers));    //放入j指标
            }
            ...
            ncolForSmith = ncolumns;
            //serialize the haed file
            head.getParentFile().mkdirs();
            try {
                ObjectOutputStream serializer = new ObjectOutputStream(new FileOutputStream(head));
                head.getParentFile().mkdirs();
                //do write
                serializer.writeInt(ncolumns);
                ...
                for (int i = 0; i < mats.length; i++) {
                    serializer.writeInt(mats[i].rows);
                    serializer.writeInt(mats[i].columns);
                }
        ...
        KhZret= "";
        int savedIdx = LoadIntMatState(1);
        for (int i = 0; i < ncolForSmith; i++) {
            if ( savedIdx > i){
                i = savedIdx-1;
                continue;
            }
            IntMatrix prevMat = null;
//            if (i>0) {
//
//            }
            List<Integer> ss = new ArrayList<>();
            List<Integer> tt = new ArrayList<>();
            int degrees[] = new int[colcopy.get(i).size()], ndeg = 0;
            int nentries[] = new int[colcopy.get(i).size()];
            ArrayList<Integer> retvals[] = new ArrayList[colcopy.get(i).size()];
            int last = 0x80000000;
            int n = 0;
            if (i != 0) {
                for (int s = 0; s < colcopy.get(i-1).size(); s++) {
                    ss.add(0);
                }
                ...
                for (int s = 0; s < colcopy.get(i-1).size(); s++) {
                    ss.set(s, colcopy.get(i-1).get(s));
                }
                for (int t = 0; t < colcopy.get(i).size(); t++) {
                    tt.set(t, colcopy.get(i).get(t));
                }

                prevMat = new IntMatrix(mats[i-1].rows, mats[i-1].columns);
                for (int ii = 0; ii < mats[i-1].rows; ii++) {
                    for (int jj = 0; jj < mats[i-1].columns; jj++) {
                        prevMat.matrix[ii][jj] = mats[i-1].matrix[ii][jj];
                    }
                }
                prevMat.source = ss;
                prevMat.target = tt;
            ...
            while (true) {
                ...
                if (min == 0x7fffffff)
                    break;
                degrees[ndeg] = min;                                    //取到了目前最小的j
                retvals[ndeg] = new ArrayList<Integer>();
                BigInteger nums[] = new BigInteger[n];                  //nums: 前一个矩阵的对角化特征值
                int nnum = 0;                                           //nnum: 非0特征值个数(rank)
                for (int j = 0; j < n; j++)
                    if (tt.get(j) == min                    //此处只计算这个固定的j指标
                            && !prevMat.matrix[j][j]
                            .equals(BigInteger.ZERO))
                        nums[nnum++] = prevMat.matrix[j][j];        //nums 决定了 Img
                nentries[ndeg] = nnum;                                  //ndeg: j指标的个数
                if (nnum != 0) {
                    boolean done = false;
                    int pk = 1;
                    while (!done) {
                        ...
                        for (;; pk++) {
                            ...
                            while (tmp > 1)
                                if (tmp % p == 0) {
                                    ...
                                    break;                              //确定指数k, 此时 p^k 恰好整除 pk
                            if (tmp == 1)
                                break;                                  //说明pk没有其他素因子, 确实形如 p^k
                        }
                        for (int j = 0; j < nnum; j++) {
                            BigInteger divmod[] = nums[j]
                                    .divideAndRemainder(BigInteger.valueOf(pk));
                            if (divmod[1].equals(BigInteger.ZERO)       //整除
                                    && !nums[j].mod(BigInteger.valueOf(pk * p))
                                    .equals(BigInteger.ZERO)) { //恰好整除
                                retvals[ndeg].add(new Integer(pk));     // retvals: Img 中的一个 p^k 指标
                                nums[j] = divmod[0];                    //商
                            }
                        }
                        done = true;
            ...
            if (i != ncolForSmith - 1) {
                ...
                if (isSparse)
                    mats[i].toSmithForm(this, i);
                else
                    mats[i].toSmithFormFast(this, i, smithState);
                // mats[i].print();
                if (!mats[i].isDiagonal())
                    throw new AssertionError("Matrix is not diagonal");
            }
            for (int j = 0; j < ndeg; j++) {						//遍历所有的j指标
                int nzeros = 0;
                if (i != ncolForSmith - 1) {
                    for (int k = 0; k < colcopy.get(i).size(); k++)
                    ...
                    for (int k = 0; k < colcopy.get(i).size(); k++)
                        if (colcopy.get(i).get(k) == degrees[j])
                            nzeros++;
                nzeros -= nentries[j];								//rank(ker/img), 即自由部分Z的个数
                ...
                if (retvals[j].size() > 0) {
                    ...
                    if (!KhZret.equals(""))
                        KhZret += " + ";
                    KhZret += "q^" + degrees[j] + "*t^" + (i + startnum) + "*Z[";
            ...
            smithState = 0;
            if (outputAlready == 1){
                File already = new File("cache/Smith/already_"+new Integer(i).toString()+".txt");
                if (!already.exists()) {
                    try {
                        already.getParentFile().mkdirs();
                        FileWriter fr = new FileWriter(already);
                        fr.write(KhZret);
                        fr.close();
                    } catch (Exception e){
                        e.printStackTrace();
    ...
    public void deLoop(int colnum) { // deloops one column
        ...
        int newn = 0;
        for (int i = 0; i < columns[colnum].n; i++) {
            System.out.print("\rreduce columns: "+colnum+"/"+ncolumns+"\t");
            System.out.print("deloop: "+i+"/"+columns[colnum].n+"                   ");
    ...
    public static Komplex<?> generateFast(int pd[][],
        ...
        System.out.println("total crossings: " + pd.length);
        ///////////////////
        invokeGC();
        ...
        for (int i = 1; i < pd.length; i++) {
            /////////////////////
            int curSize = 0;
            for (SmoothingColumn column : kom.columns) {
                curSize += column.smoothings.size();
            }
            System.out.println("add crossing " + (i+1) + "\tsize: " + curSize);
            ///////////////////////
            boolean dryRun = false;

            if (caching) {
                for (int w = pd.length; w > i; w--) {
                    if (new File("cache/" + new Integer(w).toString()).exists()) {
                        System.out.println("More cache files exist, skipping ahead at crossing: "
                                + i);
                        dryRun = true;
                        break;
                    }
...
//				}
                ...
                if (!dryRun && cache.exists()) {
                    try {
                        ObjectInputStream deserializer = new ObjectInputStream(
                                new BufferedInputStream(new FileInputStream(cache), 1024 * 1024 * 512));
            ...
            if (caching) {
                if (!dryRun) {
                    System.out.println("writing cache: " + i + " ...");
                    writeCache(kom, i);
//					if (i%5==0)
//                    {
//                        input.next();
//                    }
    ...
    public Komplex<R> compose(int start, Komplex<R> kom, int kstart, int nc,
        ...
        for (int i = 0; i < ret.ncolumns; i++) {
            System.out.print("\rcompose columns: "+i+"/"+ret.ncolumns+"                                         ");
    ...
    private void readObject(ObjectInputStream s) throws IOException,
        ...
        if (serializationVersion == 1) {
            ...
            if (!inMemory) {
                for (int i = 0; i < size; ++i) {
                    System.out.print("\rReading height " + (i + 1) + "/" + size + "                        ");
                    long fileLength = s.readLong();
                ...
                for (int i = 0; i < size; ++i) {
                    System.out.print("\rread col:"+i+"/"+size+"                                         ");
